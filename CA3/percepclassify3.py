...
word_pos_neg_weights = dict()
word_tru_dec_weights = dict()
pnBias = 0
tdBias = 0

def readParametersFromModel(fname):
    global pnBias
    global tdBias
    if 'vanilla' in fname:
        count = 1
        with open ("vanillamodel.txt") as f:
            for line in f:
                arr = line.split(':')
                if count == 1:
                    pnBias = float(arr[1].strip())
                elif count == 2:
                    tdBias = float(arr[1].strip())
                elif count > 3:
                    values = arr[1].split(",")
                    word = arr[0].strip()
                    pnweight = float(values[0].strip())
                    tdweight = float(values[1].strip())
                    word_pos_neg_weights[word] = pnweight
                    word_tru_dec_weights[word] = tdweight
                count += 1

    elif 'average' in fname:
        count = 1
        with open("averagemodel.txt") as f:
            for line in f:
                arr = line.split(':')
                if count == 1:
                    pnBias = float(arr[1].strip())
...
def classifyData(all_files):
    for file in all_files:
        f = open(file, 'r')
        reviews = f.read()
        reviews = processData(reviews)
        wordCountDict = populateWordCountDict(reviews)
        pnactivation = calculateActivation(wordCountDict, word_pos_neg_weights)
        pnactivation += pnBias
        tdactivation = calculateActivation(wordCountDict, word_tru_dec_weights)
        tdactivation+= tdBias
        label_a = ""
        label_b = ""
        if pnactivation > 0:
        ...
        if tdactivation > 0:
            ...
            label_b += "deceptive"
        fout.write(label_b+ " "+label_a+ " "+file+"\n")



modelFileName = sys.argv[1]
readParametersFromModel(modelFileName)
all_files = glob.glob(os.path.join(sys.argv[2], '*/*/*/*.txt'))
fout = open('percepoutput.txt', 'w')
classifyData(all_files)
