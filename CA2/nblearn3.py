...
for file in all_files:
    ...
    if type1 == 'positive_polarity' and type2 =='deceptive_from_MTurk':
        f = open(file, 'r')
        data = f.read()
        cntPosDec+=1
        all_List.append((data, 'positive', 'deceptive'))
    elif type1 =='positive_polarity' and type2 == 'truthful_from_TripAdvisor':
        f = open(file, 'r')
        data = f.read()
        cntpostru+=1
        all_List.append((data, 'positive', 'truthful'))
    elif  type1 =='negative_polarity' and type2 =='deceptive_from_MTurk':
        f = open(file, 'r')
        data = f.read()
        cntnegdec+=1
        all_List.append((data, 'negative', 'deceptive'))
    elif  type1 =='negative_polarity' and type2 =='truthful_from_Web':
        f = open(file, 'r')
        data = f.read()
        cntnegtru+=1
        all_List.append((data, 'negative', 'truthful'))
...
def processData():
    ...
    for item in all_List:
        ...
        out = item[0].translate(translator)
        type1 = item[1]
        type2 = item[2]
        ...
        for w in wordList:
            if w != "":
                ...
                elif type2 == 'deceptive':
                    if w in deceptiveDict:
                        count = deceptiveDict[w]
                        count += 1
                        deceptiveDict[w] = count
                    else:
                        deceptiveDict[w] = 1
                if w in commonDict:
                    count = commonDict[w]
...
def calPosProb(totalWords):
    totalPosWords = sum(posDict.values())
    return totalPosWords/totalWords

def calNegProb(totalWords):
    totalNegWords = sum(negDict.values())
    return totalNegWords/totalWords

def calDecProb(totalWords):
    totalDecWords = sum(deceptiveDict.values())
    return totalDecWords/totalWords

def calTruProb(totalWords):
    totalTruWords = sum(truthDict.values())
    return totalTruWords/totalWords

def calculatePriorProb():
    totalWords =sum(commonDict.values())
    posProb  = calPosProb(totalWords)
    classPrior['positive'] = posProb
    negProb = calNegProb (totalWords)
    classPrior['negative'] = negProb
    decProb = calDecProb(totalWords)
    classPrior['deceptive'] = decProb
    truProb = calTruProb(totalWords)
    classPrior['truthful'] = truProb


def writeToModel():
    mf = open('nbmodel.txt', 'w')
    mf.write("CommonDict Size -")
    mf.write(str(len(commonDict)))
    mf.write("\n")
    mf.write ("Class Prior Probablities\n")
    for key, value in classPrior.items():
        mf.write(key)
        mf.write(":")
        mf.write(str(value))
        mf.write("\n")
    mf.write("Vocab Sizes \n")
    for key, value in vocabSizes.items():
        mf.write(key)
        mf.write(":")
        mf.write(str(value))
        mf.write("\n")
    mf.write("Word Count232 \n")
    mf.write("Word, Positive, Negative, Deceptive, Truthful \n")
    for key, value in finalModelDict.items():
        mf.write(key)
        mf.write(":")
        mf.write(str(value))
        mf.write("\n")
...
def calVocabSizes():
    vocabSizes['posVocabSize'] = sum(posDict.values())
    vocabSizes['negVocabSize'] = sum(negDict.values())
    vocabSizes['decVocabSize'] = sum(deceptiveDict.values())
    vocabSizes['truVocabSize'] = sum(truthDict.values())
    vocabSizes['commonVocabSize'] = sum(commonDict.values())
...
def wordCount():
    for key, value in commonDict.items():
        ...
        if key in negDict:
            ...
            neg = str(0)
        if key in deceptiveDict:
            dec = deceptiveDict[key]
        else:
            dec = str(0)
        if key in truthDict:
            tru = truthDict[key]
            ...
            tru = str(0)
        finalModelDict[key] = str(pos)+','+str(neg)+','+str(dec)+','+str(tru)
